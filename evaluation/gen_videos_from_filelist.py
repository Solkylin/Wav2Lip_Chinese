#通过命令行运行，需要指定文件列表、结果目录、数据根目录和模型检查点路径等参数 例如：
#python gen_videos_from_filelist.py \
#  --filelist /path/to/your/filelist.txt \     指向包含音频和视频文件对应关系的文件列表的路径
#  --results_dir /path/to/save/results \      用于存储生成的同步视频的目录路径
#  --data_root /path/to/your/data_root_directory \     数据的根目录路径，音频和视频文件将基于这个路径被解析
#  --checkpoint_path /path/to/your/model_checkpoint.pth     预训练 Wav2Lip 模型的检查点文件路径


#参数解析  使用 argparse 解析命令行参数。这些参数包括文件列表的路径、结果保存目录、数据根目录、模型检查点路径等。
#get_smoothened_boxes  平滑检测到的面部边界框，以减少帧之间的抖动。
#face_detect  使用提供的面部检测器在一批图像中检测面部。
#datagen  数据生成器，它为 Wav2Lip 模型的推理准备批量的图像和对应的 Mel 频谱图。
#_load 和 load_model  辅助函数，用于加载预训练的 Wav2Lip 模型。
#main  主函数，执行以下步骤：
#读取文件列表并遍历其中的每一行。
#从每一行中提取音频和视频文件路径。
...
#读取视频，提取帧。
#对每个视频帧进行面部检测。
#使用数据生成器准备数据，然后使用 Wav2Lip 模型生成唇形同步的视频帧。
#将生成的视频帧写入临时视频文件。
#使用 ffmpeg 将生成的视频和原始音频合并，保存最终的同步视频。
